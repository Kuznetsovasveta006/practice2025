...
class LeftPanel(tk.Frame):
    ...
    def set_default_values(self):
        # Если размер графа известен - используем вычисленные значения
        if ParameterConfig.graph_size > 0:
            for key, entry in self.entries.items():
                default_value = ParameterConfig.get_default(key)
                entry.delete(0, tk.END)
                entry.insert(0, default_value)
        # Иначе используем базовые значения
        else:
            for key, entry in self.entries.items():
                default_value = ParameterConfig.DEFAULTS.get(key, "")
                entry.delete(0, tk.END)
                entry.insert(0, default_value)
